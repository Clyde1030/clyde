...
def main():
    ...
    print('THIS PROGRAM WILL DELETE ALL FILES FROM THE SAVE LOCATION')
...
def formatFiles(fileList):
    ...
    for src, dst in fileList:
        ...
        src = src.lower()
        try:
            if src.endswith('_transactions3.csv'):
                LOGGER.debug('Formatting ' + src + ' as transactions3 file')
                format(src, dst, fmt_trans)
            elif src.endswith('_policies4.csv'):
                LOGGER.debug('Formatting ' + src + ' as policies4 file')
                format(src, dst, fmt_pol4)
            elif src.endswith('_policies5.csv'):
                LOGGER.debug('Formatting ' + src + ' as policies5 file')
                format(src, dst, fmt_pol5)
            elif src.endswith('_modeling4.csv'):
                LOGGER.debug('Formatting ' + src + ' as modeling4 file')
                format(src, dst, fmt_mod)
            elif src.endswith('_avrf2.csv'):
                LOGGER.debug('Formatting ' + src + ' as avrf2 file')
...
def fmt_pol5(row):
    ...
    row[26] = formatMoney(row[26])
    row[27] = formatMoney(row[27])
    row[28] = formatMoney(row[28])
    row[29] = formatMoney(row[29])
    row[30] = formatMoney(row[30])
    return row
