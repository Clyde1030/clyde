...
sub Main(pmWb as workbook)
	Dim path as string: path = pm.quarterPath & "Data\MAG\" & pm.year & "Q" & pm.quarter & " Magnastar Settlement SLD.xlsx"
	...
	Qdata.Clear
	'Update links
	wb.sheets("YTD Settlement").Range("D77").Formula = "='J:\Acctng\QuarterClose\" & pm.year & "\Q" & pm.quarter & "\Data\MAG\[" & pm.year & "Q" & pm.quarter & " SLD DAC Tax.xlsx]M Summary'!$C$4"
	'Update YTD Database formula
	if pm.quarter <> 1 then
		wb.Sheets("YTD Database").Range("Z:AC").Replace What:="Q" & pm.quarter - 1, Replacement:="Q" & pm.quarter, LookAt:=xlPart, SearchOrder:=xlByRows, MatchCase:=False, SearchFormat:=False, ReplaceFormat:=False, FormulaVersion:=xlReplaceFormula2
	End if
	'#########################################################
	...
	Call SQL(workingdir & "MagYRTdb11.sqL", pm.server2,pm.database2,"SLD",MagYRT)
	Call SQL(workingdir & "MagCombined.sqL", pm.server,pm.database,"SLD",MagTrialBalance,MagOverhead,QData)
