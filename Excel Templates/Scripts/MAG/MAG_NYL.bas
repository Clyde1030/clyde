...
sub Main(pmWb as workbook)
	Dim path as string: path = pm.quarterPath & "Data\MAG\" & pm.year & "Q" & pm.quarter & " Magnastar Settlement NYL.xlsx"
	...
	Qdata.Clear
	'Update links
	wb.sheets("YTD Settlement").Range("D77").Formula = "='J:\Acctng\QuarterClose\" & pm.year & "\Q" & pm.quarter & "\Data\MAG\[" & pm.year & "Q" & pm.quarter & " NYL DAC Tax.xlsx]M Summary'!$C$4"
	...
	Call SQL(workingdir & "MagYRTdb11.sqL", pm.server2,pm.database2,"NYL",MagYRT)
	Call SQL(workingdir & "MagCombined.sqL", pm.server,pm.database,"NYL",MagTrialBalance,MagOverhead,QData)
