...
sub Main(pmWb as workbook)
	...
	Qdata.Clear
	'Update links
	wb.sheets("YTD Settlement").Range("D77").Formula = "='J:\Acctng\QuarterClose\" & pm.year & "\Q" & pm.quarter & "\Data\MAG\[" & pm.year & "Q" & pm.quarter & " JH DAC Tax.xlsx]M Summary'!$C$4"
	...
	'JH gets YRT premium detail
	Call SQL(workingdir & "MagYRTdb11_JH.sqL", pm.server2,pm.database2,"JHL",MagYRT,MagYRT.offset(10,0))
	Call SQL(workingdir & "MagCombined.sqL", pm.server,pm.database,"JHL",MagTrialBalance,MagOverhead,QData)
	wb.save	
	
	'Format JHL data as numbers
	For Each x in QData
	...
	'Adjust Ranges for JHN data
	Dim JHN_QRow, JHN_OverheadRow as Integer
	JHN_QRow = QData.End(xldown).Row -4		
	JHN_OverheadRow = MagOverhead.End(xldown).Row -4
		
	Set QData = QData.offset(JHN_QRow,0)
	'Set MagOverhead = MagOverhead.offset(JHN_OverheadRow,0)
	Set MagOverhead = wb.Sheets("Allocations").Range("A5").offset(JHN_OverheadRow,0)
	Set MagTrialBalance = MagTrialBalance.offset(0,7)
			
	'Run for JHN and paste below JHL
	Call SQL(workingdir & "MagCombined.sqL", pm.server,pm.database,"JHN",MagTrialBalance,MagOverhead,QData)
