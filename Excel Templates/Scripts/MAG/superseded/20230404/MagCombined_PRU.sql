...
--declare @carrierId varchar(3); set @carrierId = 'PR2'
...
--##############################################################################
--12/6/2022 YSL: Added left join on dbo.F15VariableSubAccount for SMF numbers	
 ,coalesce(smffee.[TotalModco Fee],0) as [totalSepManFundFee]
--##############################################################################
...
----##############################################################################
----12/6/2022 YSL: Added SMF number by policy	
	left outer join (	
		select 
			b.policyNumber	
			,sum(b.[TotalModco Fee]) as [TotalModco Fee]
		from
			(select 
				smf.policyNumber
				,smf.subAccountFundValue * coalesce(ex.percentOfSmfAVIFAllowance,1)  
					+ case when DATEFROMPARTS(year(@startdate), mindate.activityMonth, 1) between @startDate and @endDate then 3000 else 0 end as [TotalModco Fee] 
			from dbo.F15VariableSubAccount smf
			inner join (select
							policyNumber
							,activityYear
							,activityMonth
						from
							(select 
								...
								,activityYear
								,activityMonth
								,RANK() over(partition by policyNumber order by activityYear, activityMonth) as rk
							from F15VariableSubAccount
							where fundId like 'SM%' 
							group by policyNumber, activityYear, activityMonth) base 
						where rk = 1) mindate on mindate.policyNumber = smf.policyNumber
			left outer join dbo.NonModco nm on nm.policyNumber = smf.policyNumber
			inner join dbo.expenses ex on ex.carrierId = smf.carrierId and year(ex.startDate)=smf.activityYear
			where smf.fundId like 'SM%' and smf.activityYear = @year and smf.activityMonth = @quarter*3) b
			group by b.policyNumber) smffee on smffee.policyNumber = f00.policyNumber 
--##############################################################################
...
where f00.carrierId = @carrierId
  and f00.activityYear = @year
  and f00.activityQuarter = @quarter
order by f00.carrierId, f00.policyNumber
