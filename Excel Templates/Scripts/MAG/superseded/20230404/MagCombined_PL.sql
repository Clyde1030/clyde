...
--declare @carrierId varchar(3); set @carrierId = 'PLN'
...
set @startdate = dateadd(q, @quarter - 1, convert(datetime, str(@year)))
set @enddate = dateadd(ms, -3, dateadd(mm, 3, @startdate))
set @fundShare = (0.2/100)/12
;


--Margin share added EB 03/2021
with m as(
	select s.policynumber
	,@fundShare/2*(
	sum(case when s.activityyear = datepart(year,@endDate) and s.activityMonth = datepart(month,@endDate) then s.subAccountFundValue else 0 end)
	+2*sum(case when s.activityyear = datepart(year,dateadd(month,-1,@endDate)) and s.activityMonth = datepart(month,dateadd(month,-1,@endDate)) then s.subAccountFundValue else 0 end)
	+2*sum(case when s.activityyear = datepart(year,dateadd(month,-2,@endDate)) and s.activityMonth = datepart(month,dateadd(month,-2,@endDate)) then s.subAccountFundValue else 0 end)
	+sum(case when s.activityyear = datepart(year,dateadd(month,-3,@endDate)) and s.activityMonth = datepart(month,dateadd(month,-3,@endDate)) then s.subAccountFundValue else 0 end)
	)
	 as 'marginShare'
	from F15VariableSubAccount s 
	where s.carrierId = @carrierId
	and s.fundType = 'V'
	group by s.policyNumber
)
...
--******************************************************************************
 ...
 --03-2021 EB: PL not carrying unearned COI, zero
 ,0 as currentGeneralAccountReserveForUnearnedCoi
...
--******************************************************************************
 ...
 --03-2021 EB: PL not carrying unearned COI, zero
 ,0 as beginningGeneralAccountReserveForUnearnedCoi
...
,CASE WHEN @carrierID LIKE 'JH%' or @carrierID like 'PL%' THEN
--  ,CASE WHEN @carrierID LIKE 'JH%' THEN
 ...
 ,m.marginshare as expenseFundMarginSharing
 
from dbo.vF00BaseCoverageQuarterly f00
  left outer join m on f00.policyNumber = m.policyNumber
  left outer join dbo.F02MonthiversaryRecord f02 
    on f02.carrierId = f00.carrierId and f02.policyNumber = f00.policyNumber
      and f02.activityYear = f00.activityYear and f02.activityMonth = f00.activityMonth
  ...
  left outer join vMtdJournalByCpaAccountNumberQuarterly j
