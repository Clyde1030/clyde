...
sub Main(pmWb as workbook)
	dim path as string: path = pm.quarterPath & "Data\MAG\" & pm.year & "Q" & pm.quarter & " Magnastar Settlement PLN.xlsx"
	...
	Qdata.Clear
	'Update links
	wb.sheets("YTD Settlement").Range("D78").Formula = "='J:\Acctng\QuarterClose\" & pm.year & "\Q" & pm.quarter & "\Data\MAG\[" & pm.year & "Q" & pm.quarter & " PLN DAC Tax.xlsx]M Summary'!$C$4"
	...
	Call SQL(workingdir & "MagYRTdb11.sqL", pm.server2,pm.database2,"PLN",MagYRT)
	'PL includes margin sharing column
	Call SQL(workingdir & "MagCombined_PL.sqL", pm.server,pm.database,"PLN",MagTrialBalance,MagOverhead,QData)
