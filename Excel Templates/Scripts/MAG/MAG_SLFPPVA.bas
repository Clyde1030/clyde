...
sub Main(pmWb as workbook)
	Dim path as string: path = pm.quarterPath & "Data\MAG\" & pm.year & "Q" & pm.quarter & " Magnastar PPVA Settlement SLF.xlsx"
	Dim workingDir as string: workingDir = PM.scriptPath & "MAG\"
	Dim wb As Workbook: Set wb = PMUtil.GetWorkbook(path)
	
	Dim SLFAdmin as Range: Set SLFAdmin = PMUtil.GetNamedRange("SLFAdmin", wb)
	Dim SLFBreakage as Range: Set SLFBreakage = PMUtil.GetNamedRange("SLFBreakage", wb)
		
	'Data goes in same place every quarter
	SLFAdmin.Clear
	SLFBreakage.Clear
		
	Call SQL(workingdir & "MagCombined_SLFPPVA.sqL", pm.server,pm.database,"SLF",SLFAdmin,SLFBreakage)
			
	For Each x in SLFAdmin
		If Isnumeric(x.value) and not isempty(x.value) Then
			x.value = 1* x.value
		End If
	Next x
	
	For Each x in SLFBreakage
		If Isnumeric(x.value) and not isempty(x.value) Then
			x.value = 1* x.value
		End If
	Next x
